  ...
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        p("The plot of the data showed an increas in mean error for the cues as the surprise rating of the cues
          increased. This answers the question of how the surprise leve of reactivation cues can change the accuracy
          of memory recall. The general trend showed an increased error rate with more surprising reactivation cues."),
        ...
        p("All of the self-report categories tended to be categroized most frequently as higher surprise, however
          they all had an average of 3. Showing that they mostly had similar averages, however fields like arousal and 
          familarity had a more intense effect on surprise than valence level.")
      )
    )
  )
...
server <- function(input, output) {
  ...
  output$interactive_plot <- renderPlot({
    ...
    study_1 %>%
      ...
      geom_violin(alpha = .8) +  # Setting alpha to .8 for aesthetics
  ...
  output$big_question_plot <- renderPlot({
    ...
    combined2() %>%
      ...
      labs(
        ...
        y = "Cue Error",
        color = "Video Cue",
        caption = "Data Source: Study 1 and Study 2"
      ) +
      theme_minimal()+
      theme(
  ...
  output$plot_interpretation <- renderText({
    ...
    interpretation <- switch(
      ...
      "Valence_1" = "Valence had a closer correlation to surprise. A valence of 1 was frequently a surprise of 1."
    )
    interpretation
  })
}
